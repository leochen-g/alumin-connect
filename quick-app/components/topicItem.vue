<template>
  <div class="activity-item shadow">
    <div class="alumni-topic-item">
      ...
      <div class="alumni-topic-header-row">
        <div class="account-group">
          ...
          <div
            ...
            @click="toUserDetail(todo.userInfo.openid)"
          >
            <img
              class="user-avataUrl"
              v-bind:src="todo.userInfo.avataUrl"
              alt=""
            />
          </div>
          <div class="header-content">
            <div
              ...
              @click="toUserDetail(todo.userInfo.openid)"
            >
              {{ todo.userInfo.nickName }}
            </div>
            <div class="user-meta-box">
              <div class="university-info ellipsis">
                {{
                  todo.userInfo.graduationTime
                    ? todo.userInfo.graduationTime
                    : '毕业时间'
                }}
                @ {{ todo.userInfo.college ? todo.userInfo.college : '院系' }}
              </div>
              <div class="dot">·</div>
              <div>
                {{ todo.userInfo.major ? todo.userInfo.major : '专业' }}
              </div>
            </div>
          </div>
        </div>
        <div class="header-action">
          <div class="more-btn" @click="moreAction(todo.id)">
          ...
          <div class="dropdown" v-if="actionId === todo.id && showHeaderAction">
            ...
            <ul class="dropdown-menu">
              <li
                v-if="openId !== todo.userInfo.openid"
                @click="tipOff(todo.id)"
              >
                举报
              </li>
              <li
                v-if="openId === todo.userInfo.openid"
                @click="deleteTopic(todo.id)"
              >
                删除
              </li>
            </ul>
        ...
        <div class="action-box">
          <div class="like-action action" @click="likeClick(todo.id)">
            <div class="action-title-box">
              <i
                class="aliiconfont action-icon"
                v-bind:class="{ 'active-action': todo.hasLiked }"
                >&#xe630;</i
              >
              <span class="action-title">{{
                todo.likeCount ? todo.likeCount : '赞'
              }}</span>
            </div>
          </div>
          <div class="comment-action action" @click="commentClick(todo.id)">
            <div class="action-title-box">
              ...
              <span class="action-title">{{
                todo.commentCount ? todo.commentCount : '评论'
              }}</span>
            </div>
          </div>
        </div>
      ...
      <!--评论列表-->
      <div
        class="alumni-topic-comment-row"
        v-if="showCommentBox && showCommentId == todo.id"
      >
        <div class="comment-list-box">
            ...
            <div class="form-box">
              ...
              <div class="comment-action-box">
                <div class="comment-submit">
                  <button class="comment-submit-btn" v-if="!commentContent">
                    评论
                  </button>
                  <button
                    class="comment-submit-btn active"
                    v-if="commentContent"
                    @click="addComment(todo.id)"
                  >
                    评论
                  </button>
                </div>
          ...
          <div class="comment-list" v-if="commentList.targetId == todo.id">
            ...
            >
              <div class="comment">
                <div
                  class="user-popover-box"
                  @click="toUserDetail(item.userInfo.openid)"
                >
                  <img
                    class="user-avataUrl comment-user-avataUrl"
                    v-bind:src="item.userInfo.avataUrl"
                    alt=""
                  />
                </div>
                <div class="comment-content-box">
                  <div class="comment-meta-box">
                    <div class="comment-user-info ellipsis">
                      <span
                        ...
                        }}<span class="comment-user-college">
                          {{
                            item.userInfo.graduationTime
                              ? item.userInfo.graduationTime
                              : '毕业时间'
                          }}
                          @
                          {{
                            item.userInfo.college
                              ? item.userInfo.college
                              : '院系'
                          }}</span
                        ></span
                  ...
                  <div class="comment-reply">
                    <div
                      class="comment-reply-action-box"
                      @click="
                        replyCommentClick(
                          item.cid,
                          item.cid,
                  ...
                  <!--回复框-->
                  <div
                    class="reply-form reply"
                    v-if="replyCommentShow && replyCommentId == item.cid"
                  >
                    <div class="form-box">
                      <div class="input-box">
                        <input
                  ...
                  <div class="sub-comment-list">
                    <div
                      class="reply-item"
                      v-for="(replyItem) in item.topComment"
                      :key="replyItem.id"
                    >
                      <div class="sub-comment">
                        <div class="sub-comment-content-row">
                          <div class="sub-comment-content-box">
                            <div
                              class="user-popover-box"
                              @click="toUserDetail(replyItem.userInfo.openid)"
                            >
                              <img
                                class="user-avataUrl comment-user-avataUrl"
                                v-bind:src="replyItem.userInfo.avataUrl"
                                alt=""
                              />
                            </div>
                            <div class="user-content-box">
                              <div class="comment-user-info ellipsis">
                                <span
                                  class="user-nickName"
                                  @click="
                                    toUserDetail(replyItem.userInfo.openid)
                                  "
                                  >{{ replyItem.userInfo.nickName
                                  }}<span class="comment-user-college">
                                    {{ replyItem.userInfo.graduationTime }} @
                                    {{ replyItem.userInfo.college }}</span
                                  ></span
                                >
                              </div>
                              <div class="comment-content">
                                ...
                                <span
                                  class="reply-user"
                                  @click="
                                    toUserDetail(replyItem.respUserInfo.openid)
                                  "
                                  >{{ replyItem.respUserInfo.nickName }}</span
                                >:{{ replyItem.content }}
                              </div>
                              <div class="sub-comment-stat-box">
                                <div
                                  class="sub-comment-action-box"
                                  @click="
                                    replyCommentClick(
                                      item.cid,
                                      replyItem.id,
                                ...
                                >
                                  <div class="sub-reply-action-comment">
                                    <i class="aliiconfont action-icon"
                                      >&#xe620;</i
                                    >
                                    <span class="action-title">回复</span>
                                  </div>
                    ...
                    <a
                      @click="gteReplyListByCommentId(item.cid, index)"
                      v-if="item.topComment.length < item.replyCount"
                      class="fetch-more-comment"
                      >加载更多</a
                    >
                    <a class="fetch-more-comment fetch-active" v-if="fetchAll"
                      >加载更多</a
                    >
                  </div>
                </div>
          ...
          <a
            @click="fetchMore(todo.id)"
            class="fetch-more-comment"
            v-if="commentCount > limit"
            >加载更多</a
          >
          <a class="fetch-more-comment fetch-active" v-if="fetchAll"
            >加载更多</a
...
export default {
  ...
  methods: {
    ...
    replyCommentClick(cid, rid, name) {
      // 点击回复图标
    ...
    addReply(cid, rid, rType, toUid, toNickName, topicId, index) {
      // 添加回复
      let _this = this;
      let req = {
        cid: cid,
    ...
    getCommentList(id, type) {
      ...
      getCommentList(req).then((res) => {
        ...
        if (type) {
          _this.commentList.comments = _this.commentList.comments.concat(
            res.data.comments
          );
        } else {
          _this.start = 0;
...
.dropdown-menu {
  ...
  box-shadow: 0 6rpx 24rpx 0 rgba(0, 0, 0, 0.06);
}
.dropdown-menu li {
  padding: 20rpx 24rpx;
  display: block;
