<template>
  <div
    class="message-main"
    @click="changeMessageStatus(todo.id, todo.topicInfo.id, todo.flag)"
  >
      ...
      <div class="message-panel">
        ...
        <div class="message-left">
          <div class="user-avataurl">
            <img
              v-bind:src="todo.producer.avataUrl"
              alt=""
        ...
        <div class="message-right">
          <div class="message-header">
            <div class="message-user-nickName">
              {{ todo.producer.nickName }}
            </div>
            <div class="message-time">{{ createTime }}</div>
          </div>
          <div class="message-content-main">
            <div class="ellipsis">
              <span v-if="todo.contentType === 'liked'"
                >赞了：{{ todo.topicInfo.content }}</span
              >
              <span v-if="todo.contentType === 'comment'"
                >评论了你的话题：{{ todo.topicInfo.content }}</span
              >
              <span v-if="todo.contentType === 'reply'"
                >回复了你的评论：{{ todo.topicInfo.content }}</span
              >
            </div>
            <span
              v-if="todo.flag"
              v-bind:class="{ 'active-msg': todo.flag }"
              class="aliiconfont"
              >&#xe643;</span
            >
          </div>
