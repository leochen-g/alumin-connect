<template>
  <div class="alumni-main">
    ...
    <swiper
      ...
      circular="true"
    >
      <block v-for="(item, index) in swiperList" :index="index" :key="index">
        <swiper-item>
          <image
            @tap="previewImg"
            :src="item.screenshot"
            :data-src="item.screenshot"
            class="slide-image"
            mode="aspectFill"
          />
          <div
            v-if="item.type === 'public' && item.position === 'top'"
            class="swiper-text"
          >
            <span class="swiper-university">《校友来了》</span><br />祝{{
              university
            }}全体校友<br />{{ item.description }}
          </div>
        </swiper-item>
      </block>
    </swiper>
    ...
    <div class="to-location-setting" v-if="authLocation">
      ...
      <div class="location-tips">
        本小程序是基于位置为你提供同城校友话题，请到设置中打开位置授权，谢谢
      </div>
    </div>
  </div>
</template>
