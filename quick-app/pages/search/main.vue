      ...
      <form action="" class="search_form">
        <div class="search_com">
          <div class="search_com_input">
            ...
            <div
              v-if="searchClearBtn && searchValue !== ''"
              @click="clearSearch"
            >
    ...
    <div class="panel" v-if="searchClearBtn && searchValue !== ''">
      <div class="panel_bg">
        <div
          ...
          :key="index"
          v-bind:style="{
            backgroundColor: chooseIndex === index ? '#999999' : '#ffffff',
          }"
          @touchstart="bindTap(index)"
          @touchend="tapOver"
          @click="choose(item.name)"
        >
          <div class="universityItem_icon">
            <i class="aliiconfont search_btn">&#xe627;</i>
          </div>
          <div class="universityItem_name">{{ item.name }}</div>
        </div>
      </div>
    </div>
	<div><ad unit-id="adunit-58745274c43058ae"></ad></div>
  </div>
</template>
